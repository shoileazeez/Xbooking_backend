"""
Daily Booking E2E API Test
Tests: Registration, Login, Space Selection, Calendar Check, Daily Booking, Guests, Payment, QR Code

Flow:
1. Register/Login User
2. List Workspaces → Select Workspace
3. List Spaces → Select Space
4. Get Calendar (daily mode for specific month)
5. Check date availability
6. Create Daily Booking (full day)
7. Add Guests immediately after booking
8. Create Order
9. Initiate Payment
10. Get Booking QR Code
11. Verify Guest QR Codes
12. Check Booking Details
"""
import requests
import json
import time
...
# Guest emails
GUEST_EMAILS = [
    "daily_guest1@test.com",
    "daily_guest2@test.com"
]
...
def run_test():
    ...
    log_file = 'test_daily_booking_output.log'
    ...
    price_per_day = space.get('price_per_day', 0)
    print_success(f"Selected Space: {space.get('name', 'Unknown')} - ₦{price_per_day}/day")

    # =========================================================================
    # 5. CHECK DAILY CALENDAR and GET HOURLY SLOTS
    # =========================================================================
    print_step("5. Check Daily Calendar & get hourly slots")
    target_date = (datetime.now() + timedelta(days=3)).date()
    month_str = f"{target_date.year}-{target_date.month:02d}"

    # First list daily availability to ensure the day exists
    cal_url = f"{BASE_URL}/api/workspace/spaces/{space_id}/calendar/?mode=daily&month={month_str}"
    print_info(f"Checking availability for: {month_str}")
    ...
    # Now get hourly slots for the target date
    ...
    if response.status_code != 200:
        print_error(f"Failed to get hourly slots: {response.text}")
        return
    ...
    if not available_slot:
        print_error("No available hourly slots found for target date")
        return
    ...
    # =========================================================================
    # 6. CREATE DAILY BOOKING (using slot_id)
    # =========================================================================
    print_step("6. Create Daily Booking using slot_id")

    booking_data = {
        "space_id": str(space_id),
        "booking_type": available_slot.get('booking_type', 'daily'),
        "slot_id": slot_id,
        "number_of_guests": 5
    }

    print_info(f"Booking Type: {booking_data['booking_type']}")
    print_info(f"Date: {date_str}")
    print_info(f"Slot: {available_slot.get('start')} - {available_slot.get('end')}")
    ...
    print_success(f"Booking Type: DAILY")
    print_success(f"Space: {space.get('name', 'Unknown')}")
    print_success(f"Date: {target_date.strftime('%Y-%m-%d')}")
    print_success(f"Duration: Full Day (9 hours)")
    ...
    print(f"\n🎉 Daily Booking E2E Test Completed!")
